...
function display_condition_lines($focus, $field, $value, $view){
    ...
    if (!is_file('cache/jsLanguage/LayoutConditions/' . $GLOBALS['current_language'] . '.js')) {
        require_once ('include/language/jsLanguage.php');
        jsLanguage::createModuleStringsCache('LayoutConditions', $GLOBALS['current_language']);
    }
    $html .= '<script src="cache/jsLanguage/LayoutConditions/'. $GLOBALS['current_language'] . '.js"></script>';

    if($view == 'EditView'){

        $html .= '<script src="modules/LayoutConditions/conditionLines.js"></script>';
        ...
        if(isset($focus->flow_module) && $focus->flow_module != ''){
            ...
            if($focus->id != ''){
                ...
                while ($row = $focus->db->fetchByAssoc($result)) {
                    ...
                    $fields = getModuleFields(getRelatedModule($focus->flow_module,$condition_name->module_path[0]), "JSON", "", array());
