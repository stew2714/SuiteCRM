    ...
    public function display()
    {
        ...
        if (!empty($_REQUEST['process'])) {
            ...
            check_logic_hook_file(
                ...
                array(
                    1,
                    'SuiteFeed old feed entry remover',
                    'modules/SuiteFeed/SuiteFeedFlush.php',
                    'SuiteFeedFlush',
                    'flushStaleEntries'
                )
            );
                ...
                if ($_REQUEST['feed_enable'] == '1') {
                    ...
                    foreach ($active_modules as $name => $is_active) {
                        ...
                        if ($is_active == '1') {
                            ...
                            SuiteFeed::activateModuleFeed($module);
                        } else {
                            // They are disabling something that was active before
                            SuiteFeed::disableModuleFeed($module);
                        }
                    }
                    ...
                    foreach ($modulesWithFeeds as $currFeedModule) {
                        SuiteFeed::disableModuleFeed($currFeedModule, false);
                    }
                }

                $admin->retrieveSettings(false, true);
                SuiteFeed::flushBackendCache();
            } else {
        ...
        $sugar_smarty->display('modules/SuiteFeed/tpls/AdminSettings.tpl');
    }
}
