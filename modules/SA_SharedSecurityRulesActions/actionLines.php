...
function display_action_lines(SugarBean $focus, $field, $value, $view){
    ...
    if($view == 'EditView'){
        $html .= '<script src="modules/SA_SharedSecurityRulesActions/actionLines.js"></script>';

        $aow_actions_list = array();

        include_once('modules/SA_SharedSecurityRulesActions/actions.php');
        ...
        if(isset($focus->flow_module) && $focus->flow_module != ''){
            ...
            if($focus->id != ''){
                $sql = "SELECT id FROM sa_sharedsecurityrulesactions WHERE sa_shared_security_rules_id = '".$focus->id."' AND deleted = 0 ORDER BY action_order ASC";
                $result = $focus->db->query($sql);

                while ($row = $focus->db->fetchByAssoc($result)) {
                    $action_name = new SA_SharedSecurityRulesActions();
