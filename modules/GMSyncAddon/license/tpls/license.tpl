...
{/literal}{/if}{literal}
    ...
    function outfitters_toggle_office365_lm()
    {
        var enabled = $('#gm_office365_plugin_lm_enable').is(':checked');
        
        if(enabled)
        {
           $('#outfitters_license_manager_section').show();
        } else {
           $('#outfitters_license_manager_section').hide();
        }
    }
    ...
    $(document).ready(function(){
        ...
        var licenseAvailable = $('#outfitters_license_key').val();
        if( (""+licenseAvailable).length==0 )
        {
            $('#gm_office365_plugin_lm_enable').prop("checked", false);
        } else {
            $('#gm_office365_plugin_lm_enable').prop("checked", true);
        }
        outfitters_toggle_office365_lm();
    });
...
<table width="100%" border="1" cellspacing="0" cellpadding="0" class="edit view">
    <tr><th align="left" scope="row" colspan="4"><h4>GrinMark Office 365 Plugin for SuiteCRM Enabled</h4></th></tr>
    <tr>
        <td align="left" scope="row" colspan="4">
            <input type="checkbox" id="gm_office365_plugin_lm_enable" onclick="javascript:outfitters_toggle_office365_lm();">&nbsp;Enable GrinMark Office 365 Web Plugin license manager</input>
        </td>
   </tr>
</table>

<div id="outfitters_license_manager_section" style="display: none">
<table width="100%" border="1" cellspacing="0" cellpadding="0" class="edit view">
...
</div><!-- id="outfitters_license_manager_section" -->
