...
class ViewLicense extends SugarView
{
    ...
    protected function _getModuleTitleParams($browserTitle = false)
    {
        ...
        return array(
           ...
           translate('LBL_GMSYNCADDON','Administration').': '.translate('LBL_GMSYNCADDON_LICENSE_TITLE','Administration'),
           );
    }
    ...
    public function display()
    {
        ...
        if(isset($outfitters_config['manage_licensed_users']) && $outfitters_config['manage_licensed_users'] == true) {
            ...
            $used_licenses = 0;
            global $db,$locale;
            $result = $db->query("SELECT users.id, users.user_name, users.first_name, users.last_name FROM gmsync_users INNER JOIN users ON gmsync_users.user_id = users.id WHERE shortname = '".$db->quote($outfitters_config['shortname'])."'",false);
