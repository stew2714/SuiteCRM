...
function display_condition_lines($focus, $field, $value, $view){
    ...
    if($view == 'EditView'){
        ...
        if(isset($focus->flow_module) && $focus->flow_module != ''){
            ...
            if($focus->id != ''){
                $sql = "SELECT id FROM sa_sharedsecurityrulesfields WHERE sa_shared_sec_rules_id = '".$focus->id."' AND deleted = 0 ORDER BY 
condition_order ASC";
                $result = $focus->db->query($sql);

                while ($row = $focus->db->fetchByAssoc($result)) {
                    $condition_name = new SA_SharedSecurityRulesFields();
        ...
        if(isset($focus->flow_module) && $focus->flow_module != ''){
            ...
            $html .= "flow_rel_modules = \"".trim(preg_replace('/\s+/', ' ', getModuleRelationships($focus->flow_module)))."\";";
            $html .= "flow_module = \"".$focus->flow_module."\";";
            $sql = "SELECT id FROM sa_sharedsecurityrulesfields WHERE sa_shared_sec_rules_id = '".$focus->id."' AND deleted = 0 ORDER BY condition_order ASC";
            $result = $focus->db->query($sql);
