...
class SecurityGroup extends SecurityGroup_sugar
{
     ...
     * @return STRING
     */
    static function getGroupWhere($table_name,$module,$user_id)
    {
     ...
     * @return STRING
     */
    static function getGroupJoin($table_name,$module,$user_id)
    {
     ...
     * @return STRING
     */
    static function getGroupUsersJoin($user_id)
    {
        return " LEFT JOIN (
    ...
    static function inherit(&$focus,$isUpdate)
    {
        ...
        if(
            ...
            if(in_array($focus->module_dir,array_keys($security_modules))) {
                ...
                $memberships = SecurityGroup::getMembershipCount($current_user->id);
                if($memberships > 1) {
                    return;
                }
            }
     ...
     */
    static function inheritOne($user_id, $record_id, $module) {
     ...
     */
    static function getMembershipCount($user_id) {
        global $db;
    ...
    static function retrieveDefaultGroups() {
        global $db;
    ...
    static function saveDefaultGroup($group_id, $module) {
        global $db;
    ...
    static function removeDefaultGroup($default_id) {
        global $db;
     ...
     */
    static function getSecurityModules() {
    ...
    static function getLinkName($this_module, $rel_module) {
     ...
     */
    static function addGroupToRecord($module, $record_id, $securitygroup_id) {
     ...
     */
    static function removeGroupFromRecord($module, $record_id, $securitygroup_id) {
        if(empty($module) || empty($record_id) || empty($securitygroup_id)) {
            return; //missing data
        }
        global $db;
     ...
     */
    static function getUserSecurityGroups($user_id)
    {
        global $db;
     ...
     */
    static function getAllSecurityGroups()
    {
        global $db;
...
order by securitygroups_users.primary_group desc ";
    ...
    public static function get_current_plan()
    {
        ...
        require_once('modules/SecurityGroups/license/SecuritySuiteOutfittersLicense.php');
        static::$current_plan = SecuritySuiteOutfittersLicense::get_current_plan('SecurityGroups');

        return static::$current_plan;
    }

    // For when certain check points happen before the framework is loaded
    public static function get_cached_current_plan()
    {
        if (!empty(static::$current_plan) && in_array(static::$current_plan,static::$plans))
        {
            return static::$current_plan;
