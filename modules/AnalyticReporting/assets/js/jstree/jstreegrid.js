	...
	$.jstree.plugins.grid = function(options,parent) {
		...
		this._prepare_headers = function() {
			...
			for (i=0;i<cols.length;i++) {
				...
				width -= borPadWidth;
				margin = i === 0 ? 3 : 0;
				// #3661 [start] - Changed .text to .html
				...
				last = $("<th></th>").css(conf).css({"margin-left": margin,"width":width}).addClass((tr?"ui-widget-header ":"")+"jstree-grid-header jstree-grid-header-cell jstree-grid-header-"+classAdd+" "+cl).html(val).appendTo(header);
				// #3661 [end]
