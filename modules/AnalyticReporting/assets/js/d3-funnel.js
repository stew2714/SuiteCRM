(function(global) {
	...
	D3Funnel.prototype.draw = function(data, options) {
		...
		var svg = d3.select(this.selector)
			...
			.attr("width", this.width + this.margin.left + this.margin.right) // #4540 - Add margins
			.attr("height", this.height + this.margin.top + this.margin.bottom) // #4540 - Add margins
		...
		for (var i = 0; i < sectionPaths.length; i++) {
			...
			var textStr = this.data[i][0] + ": " + numberFormat(null, null, this.data[i][1]);
	...
	D3Funnel.prototype._initialize = function(data, options) {
		...
		var settings = jQuery.extend({}, this.defaults);
		settings.width  = jQuery(this.selector).width();
		settings.height = jQuery(this.selector).height();
		...
		this.width = settings.width - this.margin.left - this.margin.right; // #4540 - Subtract margins
		this.height = settings.height - this.margin.top - this.margin.bottom; // #4540 - Subtract margins
