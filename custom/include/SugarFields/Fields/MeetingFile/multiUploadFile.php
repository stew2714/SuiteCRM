...
class MultiUploadFile
{
     ...
     */
    public function __construct($field_name = '', $key = '')
    {
     ...
     */
    public function MultiUploadFile($field_name = '')
    {
        $deprecatedMessage =
    ...
    public function confirm_upload()
    {
        ...
        if ($_FILES[$this->field_name]['error'][$this->key] != UPLOAD_ERR_OK) {
            if ($_FILES[$this->field_name]['error'][$this->key] != UPLOAD_ERR_NO_FILE) {
                if ($_FILES[$this->field_name]['error'][$this->key] == UPLOAD_ERR_INI_SIZE) {
                    ...
                    $errMess =
                        string_format(
                            ...
                            array(
                                $_FILES['filename_file']['error'][$this->key],
                                self::$filesError[$_FILES['filename_file']['error'][$this->key]],
        ...
        if (!is_uploaded_file($_FILES[$this->field_name]['tmp_name'][$this->key])) {
            return false;
        } elseif ($_FILES[$this->field_name]['size'][$this->key] > $sugar_config['upload_maxsize']) {
        ...
        $this->temp_file_location = $_FILES[$this->field_name]['tmp_name'][$this->key];
        $this->uploaded_file_name = $_FILES[$this->field_name]['name'][$this->key];
    ...
    {
        $filename = $_FILES_element['name'][$this->key];
        $filetype = isset($_FILES_element['type'][$this->key]) ? $_FILES_element['type'][$this->key] : null;
        ...
        if ($filetype && !$recheckMime) {
            ...
            $mime = mime_content_type($_FILES_element['tmp_name'][$this->key]);
        } elseif ($is_image) {
            $info = getimagesize($_FILES_element['tmp_name'][$this->key]);
            if ($info) {
                $mime = $info['mime'];
            }
    ...
    function create_stored_filename()
    {
        ...
        if (!$this->use_soap) {
            $stored_file_name = $_FILES[$this->field_name]['name'][$this->key];
            ...
            if (is_windows()) {
                ...
                $this->original_file_name = $_FILES[$this->field_name]['name'][$this->key];
            }
    ...
    function final_move($bean_id)
    {
        ...
        if ($this->use_soap) {
            ...
            if (!UploadStream::move_uploaded_file($_FILES[$this->field_name]['tmp_name'][$this->key], $destination)) {
                $GLOBALS['log']->fatal(
                    "ERROR: can't move_uploaded_file to $destination. You should try making the directory writable by the webserver"
                );
