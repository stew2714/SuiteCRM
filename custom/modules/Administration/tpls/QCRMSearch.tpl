...
{foreach name=tabbasic_search from=$tabbasic_search key=tabColkey item=tabbasic_searchData}
					<li id= '{$tabbasic_searchData.field}' class='ui-state-highlight'>{$tabbasic_searchData.label}{if $showfields}<span class="field_label" style="font-size:smaller;"><br>({$tabbasic_searchData.field})</span>{/if}</li>
{/foreach}
			</ul>
		</div>
		<div id="selected_div">
