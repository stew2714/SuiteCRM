...
class AORReportsDashlet extends Dashlet
{
    ...
    public function display()
    {
        ...
        $dashletSmarty->assign('viewing_as', $this->view_as);
    ...
    public function displayOptions()
    {
        ...
        foreach($groups as $secGroup){
            $secGroup = BeanFactory::getBean("SecurityGroups", $secGroup['id']);
            if($secGroup != false && $secGroup->load_relationship("users")){
             $users = $secGroup->users->getBeans();
             foreach($users as $user){
                 if($current_user->id != $user->id){
                     $array[$user->id] = $user->user_name;
                 }
             }
            }
        }
        }else{
            $userBean = BeanFactory::getBean("Users", $this->view_as);
            $results = $userBean->get_full_list();
        ...
        //end of extras.
        $optionsSmarty->assign('userList', get_select_options_with_id($array, $this->view_as));
